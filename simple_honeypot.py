...
class CONNECTION_OPS():
    ...
    def SOCKET_CONNECTION(lhost=str,lport=int,lcount=int,dsize=int):
        ...
        while True:
            ...
            banner_message = '220 ProFTPD 1.2.8 Server\nHEREName: '
            try:
                ...
                r_host = ATTRIBUTES_CONNECTION.INFO_LOG_HOST("")
                r_port = ATTRIBUTES_CONNECTION.INFO_LOG_PORT(21)
                CONNECTION_OPS.SEND_LOG_INFO(inner_add[0],f"ERROR: {str(sc_)}",r_host,r_port,lport)
            else:
                r_host = ATTRIBUTES_CONNECTION.INFO_LOG_HOST("")
                r_port = ATTRIBUTES_CONNECTION.INFO_LOG_PORT(21)
                CONNECTION_OPS.SEND_LOG_INFO(inner_add[0],response_message,r_host,r_port,lport)
            finally:
                inner_u.close()
                sc_main.close()
    def MAIN_OPS():
        ...
        l_port = ATTRIBUTES_CONNECTION.LOCAL_PORT(21)
